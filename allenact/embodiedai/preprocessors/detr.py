...
from submodules.detr.apis.inference import (
    inference_detector,
    init_detector,
...
class DETREmbedder(nn.Module):
    def __init__(self, detr_path: Optional[str] = None):
        ...
        self.detector, self.postprocessor = init_detector(args)
    ...
    def embed_detection_results(self, scores, labels, boxes, output, target):
        ...
        current_detection_features[
            (sorted_labels == (len(TARGETS) + 1)).expand_as(current_detection_features)
        ] = 0
        ...
        detection_inputs["indicator"] = (
            ...
            == einops.repeat(target, "b -> b n 1", n=100).expand_as(
                detection_inputs["labels"]
            )
        ).float()
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            result = self.postprocessor["bbox"](
                output,
                einops.repeat(
                    self.image_size, "1 s -> b s", b=output["pred_logits"].shape[0]
                ),
            )

            pred_scores = torch.cat(
                [einops.rearrange(x["scores"], "l -> 1 l 1") for x in result], dim=0
            )
            pred_labels = torch.cat(
                [einops.rearrange(x["labels"], "l -> 1 l 1") for x in result], dim=0
            )
            pred_boxes = torch.cat(
                [einops.rearrange(x["boxes"], "l b -> 1 l b") for x in result], dim=0
            )

            detection_outputs = self.embed_detection_results(
                pred_scores, pred_labels, pred_boxes, output, x["goal_object_type_ind"]
            )
