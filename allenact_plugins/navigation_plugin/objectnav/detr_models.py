...
class ResnetDETRTensorGoalEncoder(nn.Module):
    def __init__(
        self,
    ...
    def adapt_input(self, observations):
        ...
        observations[self.detr_uuid] = {
            k: v.view(-1, *v.shape[-2:]) for k, v in detr.items()
        }
    ...
    def forward(self, observations):
        ...
        embs, _ = self.visual_transformer(
            self.compress_detr(observations), resnet_features
        )
