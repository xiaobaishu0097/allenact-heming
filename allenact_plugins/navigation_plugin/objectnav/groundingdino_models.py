...
class ResnetGroundingDINOTensorGoalEncoder(nn.Module):
    ...
    def __init__(
            ...
            groundingdino_preprocessor_uuid: str,
        ...
        self.groundingdino_uuid = groundingdino_preprocessor_uuid
        ...
        if not self.blind:
            ...
            self.resnet_compressor = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Conv2d(self.resnet_hid_out_dims[0],
                          self.resnet_hid_out_dims[0], 1),
                nn.ReLU(),
            )
    ...
    def compress_groundingdino(self, observations) -> torch.Tensor:
        groundingdino_feature = observations[self.groundingdino_uuid]
        groundingdino_feature = self.groundingdino_compressor(
            groundingdino_feature)
        return groundingdino_feature

    def process_resnet_groundingdino(self, resnet_feature,
                                     groundingdino_feature) -> torch.Tensor:
        pass

    def adapt_input(self, observations):
        resnet = observations[self.resnet_uuid]
        groundingdino = observations[self.groundingdino_uuid]
    ...
    def forward(self, observations):
        ...
        resnet_features = einops.rearrange(
            ...
            'b c h w -> b c (h w)')
        embs, _ = self.visual_transformer(
            self.compress_groundingdino(observations), resnet_features)
        x = self.target_obs_combiner(
            einops.rearrange(embs, 'b (h w) c -> b c h w', h=7, w=7))
        x = x.reshape(x.size(0), -1)  # flatten

        return self.adapt_output(x, use_agent, nstep, nsampler, nagent)


class ResnetDualGroundingDINOTensorGoalEncoder(nn.Module):
...
class ResnetGroundingDINOTensorNavActorCritic(VisualNavActorCritic):
    ...
    def __init__(
        ...
        if (rgb_resnet_preprocessor_uuid is None
            ...
            self.goal_visual_encoder = ResnetGroundingDINOTensorGoalEncoder(
                self.observation_space,
                goal_sensor_uuid,
                resnet_preprocessor_uuid,
                rgb_groundingdino_preprocessor_uuid,
                goal_dims,
